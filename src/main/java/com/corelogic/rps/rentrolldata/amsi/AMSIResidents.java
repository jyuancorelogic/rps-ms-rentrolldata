...
public class AMSIResidents {
   ...
   public String  getResidentsByStatusChangeOrTransactionDateForResidents(AMSIInputs inputs) {			
	   ...
	   try {
		   ...
		   String proprequest=AMSIUtil.processHR(inputs.getPropertyId(),inputs.getStartFromDate(),inputs.getStartToDate(),inputs.getTranFromDate(),inputs.getTranToDate());
		   result=port.getResidentsByStatusChangeOrTransactionDate(inputs.getUsername(), inputs.getPassword(), inputs.getPortfolio(), proprequest);
